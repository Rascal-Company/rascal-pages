...
Rascal Pages is a multi-tenant landing page builder SaaS built with Next.js 16 (App Router), React 19, Supabase (auth + PostgreSQL), and Tailwind CSS 4.

### Multi-Tenant Routing

The `middleware.ts` handles all multi-tenant routing based on hostname:

| Hostname | Routes to | Purpose |
|----------|-----------|---------|
| `app.rascalpages.fi` | `/app/*` | Dashboard & editor |
| `rascalpages.fi`, `www.*` | `/home` | Marketing site |
| `*.rascalpages.fi` | `/sites/[subdomain]` | Published user sites |
| Custom domains | `/sites/[domain]` | Published user sites |
...
Five landing page templates defined in `src/lib/templates.ts`:
- `saas-modern` (default) - Hero, features grid, FAQ
- `lead-magnet` - Left features, right form/image
- `waitlist` - Minimal centered with input
- `vsl` - Video sales letter format
- `personal` - Hero, about, testimonials

Content is stored as JSONB `TemplateConfig` with fields: `templateId`, `hero`, `features`, `testimonials`, `faq`, `about`, `videoUrl`, `theme`.

### Key Directories

- `app/app/dashboard/` - Dashboard routes (force-dynamic)
- `app/components/editor/` - Page editor with field components
- `app/components/templates/` - Template rendering components
- `app/components/renderer/SiteRenderer.tsx` - Routes content to correct template
- `app/actions/` - Server Actions for mutations
- `src/lib/templates.ts` - Template definitions (SSOT)
- `src/lib/supabase/` - Supabase client factories

### Server Actions

Server Actions in `app/actions/` handle:
- `createSite` - Creates site with subdomain validation
- `updatePageContent` - Saves page content with ownership check
- `togglePagePublish` - Toggles publish status

### Data Flow

Editor uses client state (`useState<TemplateConfig>`) with immutable update functions in `contentUpdaters.ts`. Changes are persisted via Server Actions which call `revalidatePath()` to refresh Next.js cache.
